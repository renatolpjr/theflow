...
import { Loader2, Plus, Pencil, Trash2, Volume2 } from 'lucide-react';
import { toast } from 'sonner';
import { ListeningExerciseFormDialog } from '@/components/admin/exercises/listening-exercise-form-dialog';
...
export default function AdminListeningExercisesPage() {
  ...
  const fetchExercises = async () => {
    try {
      const response = await fetch('/api/admin/listening-exercises');
  ...
  const handleToggleActive = async (id: string, currentStatus: boolean) => {
    try {
      const response = await fetch(`/api/admin/listening-exercises/${id}`, {
        method: 'PUT',
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-7xl">
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-slate-900">Exercícios de Listening</h1>
          <p className="text-slate-600 mt-1">Gerencie os exercícios de compreensão auditiva</p>
        </div>
        ...
        <CardContent>
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="text-slate-700">Voz</TableHead>
                <TableHead className="text-slate-700">Velocidade</TableHead>
                <TableHead className="text-slate-700">Perguntas</TableHead>
            ...
            <TableBody>
              {exercises.map((exercise) => (
                <TableRow key={exercise.id}>
                  ...
                  <TableCell className="text-slate-700">{exercise.voice}</TableCell>
                  <TableCell className="text-slate-700">{exercise.speed}x</TableCell>
                  <TableCell className="text-slate-700">{Array.isArray(exercise.questions) ? exercise.questions.length : 0}</TableCell>
          ...
          {exercises.length === 0 && (
            <div className="text-center py-12">
              <Volume2 className="mx-auto h-12 w-12 text-slate-400" />
              <h3 className="mt-2 text-sm font-semibold text-slate-900">Nenhum exercício</h3>
              <p className="mt-1 text-sm text-slate-500">
                Comece criando um novo exercício de listening.
              </p>
              <div className="mt-6">
                <Button onClick={handleCreateNew} className="bg-flow-blue hover:bg-blue-700">
                  <Plus className="mr-2 h-4 w-4" />
